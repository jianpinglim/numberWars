...
const game = {
    gridSize: 9,
    blackCellCount: 16, // Number of black cells to spawn
    currentPlayer: 'blue',
    selectedCells: [],
    grid: [],
    timeLeft: 120,
    ...
    createGrid() {
        ...
        // Randomly select positions for black cells
        const blackPositions = new Set();
        while(blackPositions.size < this.blackCellCount) {
            const randomIndex = Math.floor(Math.random() * positions.length);
            // Avoid placing black cells in first and last rows
            if (Math.floor(positions[randomIndex] / this.gridSize) !== 0 && 
                Math.floor(positions[randomIndex] / this.gridSize) !== this.gridSize - 1) {
                blackPositions.add(positions[randomIndex]);
            }
        }
        ...
        for (let i = 0; i < this.gridSize * this.gridSize; i++) {
            if (blackPositions.has(i)) {
                this.grid[i] = 'X'; // Mark as black cell
            } else {
                this.grid[i] = Math.floor(Math.random() * 9) + 1;
            }
        }
    ...
    isValidSelection(index) {
        if (this.grid[index] === 'X') return false;
